/*
 File:              mazesolver.c
 Purpose:           recursively solves a maze 
...
maze_cell** parse_maze( FILE * maze_file, int dimension )
{
	...
	maze = (maze_cell **)calloc(dimension, sizeof(char*));													//unsure
																										//unsure

    for ( row = 0; row < dimension; ++row ) {
        
		maze[row] = (maze_cell*)calloc(dimension, sizeof(char));												//unsure
	}
...
/**
 ...
 PRE:       dimension contains the correct dimension of the mpaths_foundaze
...
void generate_all_paths( maze_cell ** maze, int dimension, int row, int column, char * path )
{
    ...
    /* Checks for base cases */// INSERT CODE HERE: Simply return if we hit one of the base cases (there are more than 1)
         // (remember to delete 1 on the next line, which is here so the incomplete program compiles)
         
	if ( row >= dimension || column >= dimension || row < 0 || column < 0 || maze[row][column].character == MAZE_WALL || maze[row][column].visited == VISITED) {
        return;
	}
...
int path_cost(char * path_string)
{
	...
	int index = 0;
	int sum=0;
	while (path_string[index++] != NULL)
		sum += path_string[index];
	return sum;
}
 ...
 */
void display_shortest_path ( )																			//use path cost
{
	// INSERT CODE HERE
	int index = 0;
	int row;
	int shortest_path_row=0;
	int tempsize = 0;
	int temp_row = 0;
	
	for (row = 0; row < paths_found; row++) {
		index = 0;
		tempsize = 0;
		{
			while (paths[row][index] != '\0') {

				index++;
			}
			if (index < tempsize || tempsize == 0) {
				temp_row = row;
				tempsize = index;
			}
		}
		...
		for (row = 0; row < paths_found; row++) {
			...
			int tempcost = 0;
			{
				while (paths[row][index] !='\0') {
					...
					tempcost += paths[row][index] -'0';
					index++;
				}
				if (tempcost < cost || cost == 0) {
					cost = tempcost;
					temp_row = row;
					tempsize = index;
				}
